...
把 **Mirabox N3** 控制台变成你的 **Linux 桌面 AI 控制台**：9 个按键 + 3 个旋钮，AI 认知发芽、提示词优化、语音输入、框选截图全部一键完成。基于 [OpenDeck](https://github.com/nekename/OpenDeck) 软件驱动，适用于 Ubuntu（X11/GNOME）桌面。
...
| 2 | <img src="icons/2.png" width="48" alt="认知发芽"> | **认知发芽助手** | 选中任何文字（想法/情绪/摘录），AI 生成结构化「认知种子」分析，弹窗审阅确认后存入 ~/认知种子/ 收集箱；判定为知识卡片/项目种子的自动单独存入「已发芽」文件夹。提示词可在 ~/.config/streamdock-n3/key2-prompt.txt 自由修改 | DeepSeek API key |
...
**2 号键（认知发芽助手）怎么用？**
选中任何一段文字（想法、情绪、计划、文章摘录、甚至混乱的表达），按一下 2 号键：

1. 屏幕右上角立即弹通知「已收到按键，正在分析…」（约 20 秒）
2. 弹出**审阅窗口**，显示原文 + 13 段结构化「认知种子」分析（核心矛盾、风险校准、红队质疑、系统路由、最小下一步等）
3. 点「**保存**」才会写入 ~/认知种子/ 当天收集箱文件；点「**丢弃**」什么都不存
4. 被 AI 判定为「知识卡片 / 项目种子」的，会额外以独立文件存入 ~/认知种子/已发芽/ 文件夹，方便以后分拣进知识库

> 纯情绪类输入会自动简化为 6 节轻量输出；误触或选到无意义内容时不会强行分析。
> 想改分析方式？直接编辑 ~/.config/streamdock-n3/key2-prompt.txt，保存即生效。
